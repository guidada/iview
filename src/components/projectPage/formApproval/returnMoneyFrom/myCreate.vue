<template>
  <Card v-if="isViewReturnMoney">
    <viewReturnMoney
      ...
      :viewInfo="viewInfo"
      :isEditReturnMoney="isEditReturnMoney"
    ></viewReturnMoney>
  </Card>
  <div v-else class="my_create_container">
    ...
    <!-- 新建按钮-->
    <div class="btn_wrap">
      <Button type="primary" @click="gotoAddReturnMoney">新建表单</Button>
    </div>
    <!-- 退认购金单列表-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 是否编辑退认购金单（只有在驳回的情况下有）
        isEditReturnMoney: false,
    ...
    methods: {
      ...
      * */
      gotoAddReturnMoney() {
        var flag = true
        this.$emit('gotoAddReturnMoney', flag)
      },
      /*
     *  页面切换的处理
     * */
